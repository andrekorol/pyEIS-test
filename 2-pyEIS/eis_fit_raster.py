...
from eis_fit_deviates import mpfit_deviates, mpfit_model
    ...
    def __init__(self, wave, ints, errs, template, parinfo):
        ...
        for ii in range(nx):
            for jj in range(ny):
                ...
                newguess = scale_guess(
                    wave_ij, ints_ij, oldguess, self.n_gauss, self.n_poly
                )
                ...
                for n in range(self.n_gauss):
                    if fpeaks[n] != 0 and fwdths[n] != 0:
                        e_int[n] = int[n] * np.sqrt(
                            (epeaks[n] / fpeaks[n]) ** 2 + (ewdths[n] / fwdths[n]) ** 2
                        )
                    else:
                        e_int[n] = np.nan
...
if __name__ == "__main__":
    ...
    plt.title(date_obs, fontsize=22)
    fits = fit.fit
    ...
    plt.tick_params(labelsize=20)
    plt.imshow(scaled, origin="lower", aspect="auto", cmap="gray")
    plt.title("Fit", fontsize=22)
    fig = plt.gcf()
    plt.show()
    fig.savefig("spectra_raster_fit.png", bbox_inches="tight", dpi=150)
